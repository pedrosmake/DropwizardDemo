    ...
    public void run(final DropwizardWebServiceConfiguration configuration,
        ...
        environment.jersey()
                .register(new AuthDynamicFeature(
                        new OAuthCredentialAuthFilter.Builder<JwtToken>().setAuthenticator(
                                        new JwtAuthenticator(jwtKey))
...
//        environment.jersey().register(new JsonProcessingExceptionMapper(true));
        ...
        environment.jersey().register(new OrderController(
                new OrderService(new OrderDao(), new OrderValidator())));
        environment.jersey()
                .register(new ProductController(
                        new ProductService(new ProductDao(),
                                new ProductValidator())));
        environment.jersey().register(
                new CustomerController(new CustomerService(new CustomerDao())));
