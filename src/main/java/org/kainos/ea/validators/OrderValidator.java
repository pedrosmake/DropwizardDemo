...
public class OrderValidator {
    public void validateOrder(OrderRequest orderRequest) throws InvalidException, DoesNotExistException {
        CustomerDao customerDao = new CustomerDao();
        try {
            Customer cust = customerDao.getCustomerById(orderRequest.getCustomerId());
            if (cust == null) {
                throw new DoesNotExistException(Entity.CUSTOMER);
            }
        } catch (SQLException e) {
            throw new InvalidException(Entity.CUSTOMER, "Can't get customer details");
        }
